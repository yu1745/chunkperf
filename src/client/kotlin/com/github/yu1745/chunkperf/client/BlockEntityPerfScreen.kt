...
class BlockEntityPerfScreen : Screen(Text.literal("Block Entity Performance")) {
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        context.drawCenteredTextWithShadow(textRenderer, "单 Block Entity 性能", width / 2, 8, 0xFFFFFF)
        ...
        context.drawTextWithShadow(textRenderer, "方块", 10, 39, 0xDDE7EE)
        context.drawTextWithShadow(textRenderer, "实例数", width / 2 - 25, 39, 0xDDE7EE)
        context.drawTextWithShadow(textRenderer, "合计平均耗时", width - 115, 39, 0xDDE7EE)
        context.fill(8, 51, width - 8, 52, 0xFF59636E.toInt())

        val rowHeight = 22
        val visibleRows = max(1, (height - 64) / rowHeight)
        scrollOffset = scrollOffset.coerceIn(0, max(0, rows.size - visibleRows))
        val end = min(rows.size, scrollOffset + visibleRows)
        var y = 56
        var hoveredRow: BeRow? = null
        for (index in scrollOffset until end) {
            val row = rows[index]
            if ((index - scrollOffset) % 2 == 1) context.fill(8, y - 2, width - 8, y + 19, 0x182F3944)
            val block = Registries.BLOCK.get(row.blockId)
            val stack = ItemStack(block.asItem())
            if (!stack.isEmpty) context.drawItem(stack, 11, y)
            context.drawTextWithShadow(textRenderer, row.count.toString(), width / 2 - 5, y + 4, 0xC8D2DC)
            context.drawTextWithShadow(
                textRenderer,
                formatMetric(row.msPerTick),
                width - 115,
                y + 4,
                if (row.msPerTick >= 1.0) 0xFF6B6B else if (row.msPerTick >= 0.1) 0xFFD166 else 0x9FE870
            )
            if (mouseX in 8 until (width - 8) && mouseY in (y - 2) until (y + 19)) hoveredRow = row
            y += rowHeight
        }
        ...
        if (rows.isEmpty()) {
            ...
            context.drawTextWithShadow(textRenderer, "${scrollOffset + 1}-${end} / ${rows.size} 类  ·  ${samples.size} 个热点实例  ·  滚轮滚动", 10, height - 12, 0xAEB8C2)
        }
